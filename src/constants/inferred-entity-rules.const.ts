...
import {A_WEATHER_ENTITY_ATTRIBUTE, WEATHER_ENTITY_ATTRIBUTE} from './weather-entity-attributes.const';

type InferredEntityRule = {
  // If the entity ID can be explicitly named, use this rule
  explicitId?: {
    entityId: string;
  };

  // If the entity ID is inferred from a pattern, use this rule
  idPattern?: {
    parentDeviceConfigProp: A_CONFIG_PROP | undefined;
    pattern: string;
  };

  // If the entity attribute is inferred from a pattern, use this rule
  attributePattern?: {
