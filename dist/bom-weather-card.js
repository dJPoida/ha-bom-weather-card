    ...
    render() {
      ...
      `;
        ...
        const forecastRows = this._forecastEvent.forecast
            ...
            .map((dayForecast) => this._renderForecastRow(new Date(dayForecast.datetime).toLocaleDateString('en-US', { weekday: 'long' }), dayForecast.templow ?? undefined, dayForecast.temperature, dayForecast.precipitation ?? 0, dayForecast.precipitation_probability ?? 0, dayForecast.condition ?? ''));
        return x `
      <div class="container">
        ${this.showTitle ? x `<div class="title">Daily forecast</div>` : E}
        <div class="forecast-grid">
          ${forecastRows.length > 0 ? forecastRows : x `<div class="loading">No future forecast available.</div>`}
        </div>
      </div>
    `;
    }
