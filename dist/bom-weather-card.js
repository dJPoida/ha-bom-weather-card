...
const DEFAULT_CARD_CONFIG = {
    ...
    [CONFIG_PROP.DAILY_FORECAST_NUMBER_OF_DAYS]: 7,
};
...
let BwcDailyForecastElement = class BwcDailyForecastElement extends r$2 {
    ...
    render() {
        ...
        const forecastRows = this.forecastData.forecast.map((dayForecast) => this._renderForecastRow(new Date(dayForecast.datetime).toLocaleDateString('en-US', { weekday: 'long' }), dayForecast.templow ?? undefined, dayForecast.temperature, dayForecast.precipitation ?? 0, dayForecast.precipitation_probability ?? 0, dayForecast.condition ?? ''));
        return x `
      <div class="container">
...
BwcDailyForecastElement.styles = i$4 `
    :host {
      color: var(--bwc-text-color);
    }
...
__decorate([
    n({ type: Object })
], BwcDailyForecastElement.prototype, "forecastData", undefined);
__decorate([
    n({ type: Boolean })
...
let BomWeatherCardEditor = class BomWeatherCardEditor extends r$2 {
    ...
    renderNumberSlider(name, label, required = false, className = undefined, minValue = undefined, maxValue = undefined) {
        return x `<div class=${classNames('item', className)}>
    ...
    renderDailyForecastOptionsPanel() {
      ...
      ${this._config[CONFIG_PROP.SHOW_DAILY_FORECAST]
            ? x `<div class="item-group level-one">
            ${this.renderNumberSlider(CONFIG_PROP.DAILY_FORECAST_NUMBER_OF_DAYS, this.localize('editor.numberOfDays'), false, undefined, 1, 7)}
          </div>`
            : E}
    </ha-expansion-panel>`;
    }
