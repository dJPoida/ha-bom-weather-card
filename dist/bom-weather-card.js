...
let BwcDailyForecastElement = class BwcDailyForecastElement extends r$2 {
    ...
    render() {
        ...
        const forecastRows = this.forecastData.forecast.map((dayForecast) => this._renderForecastRow(new Date(dayForecast.datetime).toLocaleDateString('en-US', { weekday: 'long' }), dayForecast.templow ?? 0, dayForecast.temperature, dayForecast.precipitation ?? 0, dayForecast.precipitation_probability ?? 0, dayForecast.condition ?? ''));
        return x `
      <div class="container">
      ...
      ${showCurrentTemp || showWeatherIcon || showTime
            ...
            ${showWeatherIcon
                ? x `<bwc-weather-icon-element
                  ...
                  .weatherIcon=${getCardEntityValueAsString(this.hass, this.cardEntities[CONFIG_PROP.WEATHER_ICON_ENTITY_ID])}
                ></bwc-weather-icon-element>`
                : E}
